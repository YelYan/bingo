...
3. **Contact details** in `lib/site.ts` — `phone` is still a fictional `555` number, and `hello@bingowebstudio.com` needs a mailbox (Cloudflare Email Routing forwards it for free). `url` is live at `https://www.bingowebstudio.com`; metadata, canonicals, sitemap and JSON-LD all derive from it.
